...
class TextToSpeechGenerator:
    ...
    def update_script_with_audio(self, script_data: Dict[str, Any], audio_files: List[str]) -> Dict[str, Any]:
        """
        Update script data with audio file paths.
        ...
        Returns:
            Updated script data with audio file references
        """
        # Create a mapping of audio files to their sections
        audio_mapping = {}
        for audio_file in audio_files:
            filename = Path(audio_file).name
            if "intro_narration" in filename:
                audio_mapping["intro_narration"] = audio_file
            elif "explanation_narration" in filename:
                audio_mapping["explanation_narration"] = audio_file
            elif "practice_mcq_question" in filename:
                audio_mapping["practice_mcq_question"] = audio_file
            elif "practice_mcq_explanation" in filename:
                audio_mapping["practice_mcq_explanation"] = audio_file
            elif "summary_narration" in filename:
                audio_mapping["summary_narration"] = audio_file
        
        # Update script data with audio file paths
        updated_script = script_data.copy()
        
        # Add audio paths to respective sections
        ...
        updated_script["metadata"]["audio_files"] = audio_files
